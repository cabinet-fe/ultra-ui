...
```
src/
├── index.ts                    # export * from './chat' + './components' + type * from './types'
├── style.ts                    # 全量样式入口
├── chat/                       # 对话核心域（UI 无关，不得 import 组件）
│   ├── types.ts                # ChatMessage / ChatTool / ChatTransport 等核心类型
...
└── components/ai-chat/
    ...
    ├── tool-call.vue           # Kimi 风格可折叠工具卡片（含 needsConfirm 确认按钮）
    ├── chat-input.vue          # 输入区（多行自适应、图片附件、发送/停止）
    ├── di.ts                   # AiChatDIKey（cls + slots 注入，支撑 tool-<name> 动态插槽）
    └── __test__/
```

## 约定

- **域划分**：`chat/` 是纯逻辑域（类型、编排、transport），禁止依赖 Vue 组件；`components/` 只放 UI，通过 `chat/` 复用核心逻辑。新能力开新顶层域目录（如 `providers/`、`tools/`），不要塞进组件目录。
