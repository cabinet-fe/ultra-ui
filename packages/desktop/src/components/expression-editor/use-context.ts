...
export function useContext(editor: LexicalEditor): {
  ...
  const pickerKeyHandlerRef = shallowRef<((e: KeyboardEvent) => void) | null>(null)

  function registerPickerKeyHandler(handler: ((e: KeyboardEvent) => void) | null) {
    ...
    return mergeRegister(
      ...
      editor.registerCommand(
        ...
        () => {
          ...
          // 检测光标是否在 '@' 左右
          const isAtTriggerLeft = textContent[cursorPosition - 1] === CONTEXT_TRIGGER_CHAR
          const isAtTriggerRight = textContent[cursorPosition] === CONTEXT_TRIGGER_CHAR
      ...
      editor.registerCommand(KEY_ENTER_COMMAND, PreventDefaultListener, COMMAND_PRIORITY_LOW),
      editor.registerCommand(KEY_ESCAPE_COMMAND, PreventDefaultListener, COMMAND_PRIORITY_LOW),
      editor.registerCommand(KEY_ARROW_UP_COMMAND, PreventDefaultListener, COMMAND_PRIORITY_LOW),
      editor.registerCommand(KEY_ARROW_DOWN_COMMAND, PreventDefaultListener, COMMAND_PRIORITY_LOW)
    )
  }
