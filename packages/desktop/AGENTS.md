...
- 错误态：form-item / table-editor 的 `.is-error` 作用域内，外壳描边与光晕一并转 `color, danger`（光晕用 `--u-focus-ring-danger`），并显式声明 `:hover` 描边——控件自身的悬浮描边与错误态描边同权重，只靠源序分胜负（见 form-item / table-editor style.scss）。
...
- **`@veltra/sheet-core` 为 optional peer**：仅 `file-viewer` 的 Excel/CSV 预览需要；未安装时该预览优雅降级，不强制下游安装 sheet-core（含其引擎依赖）/ hucre。预览器对模型走动态 `import('@veltra/sheet-core')`、对 `SheetGrid` 走 `import('@veltra/sheet-core/grid')`，避免加载 desktop 主入口时解析失败。
...
- **`@codemirror/*` 打包进产物**（声明在根 `package.json` devDependencies + `pack.deps.alwaysBundle`，desktop 包内不声明），下游无需安装 codemirror，从根本上避免多实例版本冲突。仍需保持精确版本号（不带 `^`/`~`），升级时手动指定具体版本号。
- **UCodeEditor 语言包按需加载**：`components/code-editor/lang-loaders.ts` 注册各语言的 `import()` loader，切换 `lang`（`v-model:lang` / `langs`）时动态加载；`alwaysBundle` 确保 chunk 仍内联进 dist。
  - 官方语言包：① 根 `package.json` devDependencies + 根 `overrides` 添加 `@codemirror/lang-xxx`（精确版本）② `types/code-editor.ts` 扩展 `CodeEditorLang` ③ `lang-loaders.ts` 添加一条 loader。
  - legacy-modes（如 bash / powershell）：① 根 `package.json` devDependencies 添加 `@codemirror/legacy-modes`（精确版本）② `langs/<name>/` 用 `StreamLanguage.define` 包装对应 mode ③ `CodeEditorLang` + `lang-loaders.ts` 注册。
