...
```
src/
├── index.ts              # 聚合导出：sheet-core 主入口白名单 re-export + tools/types/vue；内置工具注册（import './tools/builtin'）
...
- 大 xlsx 在 Web Worker 解析（`vue/popups/import.worker.ts`：深导入
...
- **dependencies**：`hucre`（`vue/popups/import.worker.ts` 动态 `import('hucre/xlsx')`）
...
- **pack entry**：`src/index.ts`、`src/vue/style.ts`、`src/vue/popups/import.worker.ts`、`src/tools/export.worker.ts`（worker 经 `new Worker(new URL())` 引用，非 import 可达——unbundle 模式下必须显式列为 entry 才会编译进 dist）
- **neverBundle**：全部 peer + `hucre`（含 `@veltra/sheet-core`）；treeshake `moduleSideEffects` 保留 `vue/style.ts` 与 `tools/builtin.ts`
