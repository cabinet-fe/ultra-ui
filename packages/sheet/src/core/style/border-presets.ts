/**
 * 边框预设生成（纯函数，无 vue 依赖）：把「全边框 / 外边框 / 下边框 / 无边框」
 ...
 * - 下边框：底行写 bottom；底行下一行邻居的 top 写 null
...
/** 边框预设 */
export type BorderPreset = 'all' | 'outer' | 'bottom' | 'none'
 ...
 * @param preset 预设：all / outer / bottom / none
...
export function buildBorderPresetItems(
  ...
  // 外边框：清选区外一圈邻居的对侧边；下边框：清底行下一行邻居的 top。
  // 全边框共享边已双写一致，不触碰选区外（同 Excel）
  if (preset === 'outer') {
    items.push(...buildNeighborNullItems(range, ['top', 'bottom', 'left', 'right'], getStyle))
