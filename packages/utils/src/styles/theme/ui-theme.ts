import { withUnit } from '../../utils/dom/style'
import { isObj, kebabCase, mergeDeep } from '../../utils/helper/data-compat'
...
export class UITheme {
  ...
  themeToDeclarationList(theme: Theme): string[] {
    ...
    for (const c of chunks) {
      lines.push(...c.split(';').map(s => s.trim()).filter(Boolean))
    }
    return lines
  }
  ...
  private static warnLegacyOnce(): void {
    ...
    console.warn(
      '[@ultra-ui/utils] Theme CSS variables now prefer the `--u-` namespace. ' +
