# 具名导出入口 `@veltra/icons` / `@veltra/icons/normal`
...
- 全仓业务图标导入改为 `import { … } from '@veltra/icons/normal'`（彩色 `…/colorful`）。
- `apps/sample/vite.config.ts`：`@veltra/icons`、`…/normal`、`…/colorful` 解析到 `packages/icons/src` 下对应 `.ts`，便于 dev。
- `icons:smoke-resolve` 从仓库根执行以便 Bun 解析 workspace；脚本增加对 `@veltra/icons/normal` 的检查。
