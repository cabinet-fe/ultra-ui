...
将 `playgrounds/desktop` 内所有 `ultra-ui` / `ultra-ui/...` 模块说明符改为显式 `@ultra-ui/desktop`、`@ultra-ui/desktop/types`、`@ultra-ui/utils`、`@ultra-ui/compositions` 等子路径；删除仅用于该别名的 `ultra-ui.ts` 与 Vite 中 `ultra-ui` 别名；为新增依赖补充 `package.json` 中的 `@ultra-ui/compositions`。

## 内容

1. 在 `playgrounds/desktop/package.json` 增加 workspace 依赖 `@ultra-ui/compositions`（`App.vue` 等将使用 `loadTheme` / `useConfig`）。
2. 修改 `playgrounds/desktop/vite.config.ts`：移除 `resolve.alias` 中与 `ultra-ui` 相关的两条规则。
3. 删除 `playgrounds/desktop/ultra-ui.ts`（不再被引用）。
4. 按文件将 import 映射为：`@ultra-ui/desktop`（组件与 `defineTableColumns` / `message` / `FormModel` 等）、`@ultra-ui/desktop/types`（`*Exposed`、`*Props`、`BatchEditFeature` 等类型）、`@ultra-ui/utils`（`bem`、`setStyles`）、`@ultra-ui/compositions`（`useComponentProps`、`useTransition`、`loadTheme`、`useConfig`）、`@ultra-ui/utils/styles/theme`（`currentTheme`、`lightTheme`、`darkTheme`、`UITheme`）；样式侧路径与现有 `@ultra-ui/desktop/components/.../style.scss` 用法对齐。
