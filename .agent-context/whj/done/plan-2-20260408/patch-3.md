# 废除 `cat-kit/fe` 子路径，拆分 `@cat-kit/core` / `cat-kit` / `@ultra-ui/utils`
...
- 链式读写、`pick`/`omit`/`objMap`/`equal`、以及与原 cat-kit v3 `Tween` 行为对齐的动画类，集中到 `@ultra-ui/utils`（`data-compat`、`tween`），避免依赖已拆包但 API 不同的 npm `@cat-kit/fe`。
