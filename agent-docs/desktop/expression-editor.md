...
- 非禁用 / 非只读时，编辑器边框盒上方常驻灰色提示「输入 @ 唤起变量面板」（独立于输入框之外，不随内容增减隐藏）；面板弹出在 `@` 字符附近（而非编辑器左缘），底部带键盘操作提示条。
- 面板键盘行为（组件内部处理，无对外 API）：`↑` / `↓` 移动高亮项（只滚动面板列表自身，不带动页面滚动）；`Enter` 选中或进入下一级；`←` 逐级模式下返回上一级、过滤模式下不拦截；`→` 分支项进入下一级；`Esc` 关闭面板（`@过滤词` 文本保留为普通字符）；`空格` / `←` / `→` 同时退出过滤模式。
- 面板鼠标行为：逐级模式下分支项右侧的展开箭头可点击进入下一级（`selectableLevels='any'` 时点击行选中分支本身、点击箭头进入下级）；面包屑可跳回任意层级。
- chip 交互：点击 chip 主体原地打开面板重选变量（面板锚定在该 chip 附近）；`Backspace` 在 chip 边界一次删除整个 chip。
