...
description: '@veltra/desktop 的表格型编辑器组件：基于 UTable 封装，v-model 绑定行对象数组，内置序号列与「删除/新增/复制」操作列，配合 #column:{key} 插槽的 model 作用域实现单元格行内编辑；readonly 只读模式下输入控件只读、操作列与空态「添加」按钮不渲染。'
...
`@veltra/desktop` 导出表格型编辑器组件 `UTableEditor`。它在内部渲染一个 `UTable`，用 `v-model`（`modelValue`）绑定行对象数组，自动附加序号列和带「删除 / 新增 / 复制」按钮的操作列；单元格编辑通过 `#column:{key}` 插槽把输入控件绑定到插槽作用域的 `model` 上。需要用户增删复制行、就地编辑单元格数据时用 `UTableEditor`；要复用同一套列与 `#column:{key}` 插槽做只读展示时设 `readonly`（输入控件只读、操作列与空态「添加」按钮不渲染）；不需要编辑形态时直接用 `UTable`（见 `agent-docs/desktop/table.md`）。
...
/** 表格型编辑器组件属性：继承 UTable 的全部属性，把 data 换成 modelValue，并新增 readonly */
...
- 非只读（`readonly` 缺省为 `false`）时列定义末尾始终追加操作列，key 固定为 `__operation`；`readonly: true` 时不追加，DOM 中无操作列表头与按钮。

操作列行为（按钮为小号圆形文本按钮，仅非只读时渲染）：
...
插槽与 `UTable` 完全一致，最常用的是 `#column:{key}`：作用域为 `{ row, rowData, column, val, model }`，`model.modelValue` 的更新直接写回 `rowData[key]`；`readonly: true` 时 `model` 携带 `readonly: true` 且不提供写回通道。另有 `#header:{key}`、`#row:expand`、`#empty`（非只读且空表时默认渲染「添加」按钮，`readonly` 下不渲染）、`#foot`、`#body`、`#append`。

## 方法与事件

- `update:modelValue`（`(value: any[]) => void`）：非只读时删除、新增、复制、空态「添加」触发（`readonly: true` 下这些入口均不渲染，事件不会触发），payload 是不可变更新产生的新数组。单元格内通过 `model` 编辑只修改行对象属性，不触发本事件。
...
> - 空态「添加」按钮只在非只读且 `modelValue.length === 0` 时渲染；列表非空后只能用操作列的「新增」按钮增行。
...
只读场景直接设 `readonly`：空态不渲染「添加」按钮。需要可编辑但自定义空态时，`#empty` 插槽会被内置空态占据（含「添加」按钮），改用 `UTable`（其 `#empty` 插槽默认渲染 `UEmpty`，可完全覆盖），或接受内置按钮行为。
