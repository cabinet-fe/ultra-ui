---
title: UTip 文字提示
description: '@veltra/desktop 导出的气泡文字提示组件。默认插槽元素为触发器，悬停或点击后在其上方/下方/左右弹出只读提示，支持 12 种 placement（direction + alignment）、5 种 type 主题配色、受控显隐、弹出延时、自定义内容标签，用于字段说明、图标释义等轻量提示。'
...
`@veltra/desktop` 导出组件 `UTip`。默认插槽的第一个元素是触发器，`trigger` 为 `'hover'`（默认）或 `'click'`；提示内容用 `content` 或 `#content` 插槽，弹出层通过 floating-ui 定位到 `direction` + `alignment` 组合的方向上，Teleport 到 `body` 下的 `#pop-container` 容器；`type` 可切换实色主题配色。UTip 只承载只读信息；需要用户「确认 / 取消」操作的用 `UPopConfirm`。
...
/** tip 类型，取值同全局 ColorType */
export type TipType = 'primary' | 'info' | 'success' | 'warning' | 'danger'

/** tip 提示组件属性 */
export interface TipProps {
  /** 受控显隐；传值即进入受控模式，显隐变化时 emit update:visible；不传则组件内部维护 */
  visible?: boolean
  /** 主题配色；不传为默认浅色卡片，传值后气泡为实色背景 + 白色文字，箭头同色 */
  type?: TipType
...
| `type`       | `'primary' \| 'info' \| 'success' \| 'warning' \| 'danger'` | —          |  否  | 实色主题配色：背景取 `--u-color-*`、文字白色、箭头同色；不传为默认浅色卡片                     |
...
import type { TipType } from '@veltra/desktop'

const types: TipType[] = ['primary', 'info', 'success', 'warning', 'danger']
</script>

<template>
  <!-- danger 为红色背景白色文字；不传 type 为默认浅色卡片 -->
  <u-tip v-for="t in types" :key="t" :type="t" content="提示内容">
    <u-button>{{ t }}</u-button>
  </u-tip>
  <u-tip type="danger" content="删除后不可恢复">
...
> - 气泡主题色用 `type`（`'primary' | 'info' | 'success' | 'warning' | 'danger'`，同 `UButton`）：实色背景 + 白色文字，箭头颜色跟随；不传 `type` 为默认浅色卡片。没有 `effect` 属性——深色/浅色二态的概念不存在，深浅由全局主题 token 决定；更细的外观定制用 `style` / `class` prop 作用到弹出层。
