...
keywords: ['sheet', '@veltra/sheet-core', 'io', '电子表格文件导入与导出（XLSX']
aliases: ['io', 'sheet']
---
...
- 保留：数字 / 字符串 / 布尔 / 错误；日期 ↔ 1900 序列（`t='d'`）；公式表达式（`f` 不含 `=`，缓存由本地引擎重算，不支持的函数 → `#ERROR!`）；合并；样式（fill / 四边 border / font / align / `numFmt` 四类 `date` / `thousands` / `cnUpper` / `fixed`，经 `StylePool` 去重）；冻结；行高；列宽；浮动图。
- 跳过：WPS 单元格内嵌图 `cellImages`；四类之外的数字格式 `numFmt`。
...
xlsx 导入导出 round-trip 保留浮动图；CSV 不携带图片、导原始值（不应用 numFmt 显示格式）。
