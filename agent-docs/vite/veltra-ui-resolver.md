...
组件表在 `@veltra/vite` 包内生成（`bun run resolver:gen`）。判定规则：`packages/{desktop,ai,sheet}/src/components` 的直接子目录同时有 `index.ts` 与 `style.ts`，取其 `U*` 值导出。未安装 `@veltra/ai` / `@veltra/sheet` 时它们是可选 peer，模板里不用对应组件即可。
