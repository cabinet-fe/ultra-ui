...
```ts
import { $n, date, object, parallel, TreeManager, vNumber, vString } from '@cat-kit/core'

const schema = object({ amount: vNumber(), label: vString() })
...
const tree = new TreeManager({ id: 'root', children: [{ id: 'a' }, { id: 'b' }] })
