...
export async function readConfig(
  configPath = path.resolve(process.cwd(), '.pe.jsonc'),
  env = process.env
) {
  let raw
  try {
    ...
    fail(
      `找不到配置文件：${configPath}（请在仓库根目录创建 .pe.jsonc 并配置 docs_server_url 与 docs_push_lib_name）`
    )
  }
  ...
  if (!/^https?:\/\//.test(serverUrl)) {
    fail(
      `配置文件 ${configPath} 中的 docs_server_url 须以 http:// 或 https:// 开头，实际值：${config.docs_server_url}`
    )
  }
  ...
  if (!/^[a-z0-9-]+$/.test(library)) {
    fail(
      `配置文件 ${configPath} 中的 docs_push_lib_name 须仅包含小写字母、数字与连字符（^[a-z0-9-]+$），实际值：${config.docs_push_lib_name}`
    )
  }
  ...
  return { serverUrl, library, token }
}
...
export function validateFrontmatter(content) {
  ...
  if (!content.startsWith('---\n') && !content.startsWith('---\r\n')) {
    errors.push(
      '开头不是独占一行的 ---（不得有前导空行、BOM 或分隔线行尾空格），视为没有 frontmatter'
    )
    return { title, errors }
  }
...
async function main() {
  ...
  const configPath = flags.config
    ...
    : path.resolve(process.cwd(), '.pe.jsonc')
  ...
  if (missing.length > 0) {
    fail(
      `验证失败，以下 ${missing.length} 篇推送后搜不到（服务端分词或索引可能异常，报给管理员）：\n${missing.join('\n')}`
    )
  }
