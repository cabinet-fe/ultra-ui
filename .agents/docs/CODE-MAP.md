...
│   ├── sheet/                # @veltra/sheet Vue 编辑器 + 报表
│   │   └── src/{components,report,tools,types}/
│   ├── ai/                   # @veltra/ai
...
│   └── server/               # report 契约 + DeepSeek 代理（同端口 8787）
...
| sheet | `packages/sheet` | USheet、报表设计器/查看器、工具系统 | `src/index.ts` |
...
| playground | `playground` | 预览 SPA + report/AI 参考实现 | `main.ts`、`playground/server/dev.ts` |
...
2. **playground 启动**：`cd playground && bun run dev` 同时拉起前端 7788 与参考服务 8787（report + `/ai`）。
3. **报表预览**：UReportDesigner/Viewer → `DataConnector` → playground `/report-api` 代理到 8787；模板与工作区在参考服务的 SQLite（`playground/server/data/`，不提交）。
4. **发版**：changeset → `bun run release`（`dev` 分支）→ GitHub Actions publish。
5. **技能同步**：库 API 变更后 `bun run skill:gen` 更新 `skills/veltra-ui`。
