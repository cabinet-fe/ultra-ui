...
`packages/sheet/src/report/` 内核（纯 TS headless）的下钻配置模型与多层下钻栈逻辑，经 `index.ts` 导出。设计器（下钻配置入口 + 对话框）与查看器（`openMode: 'switch'` 单击下钻）均已消费本内核；`'dialog'` 打开方式查看器尚未接线。
...
- 查看器接线：`ReportViewerProps` 新增 `resolveTemplate?`——缺省时下钻不生效（无点击行为与可点视觉提示）。`useReportViewer` 持下钻栈与命中索引（随每次成功取数重建），导出 `currentTemplate`（栈顶，未下钻为 `props.template`）/ `drillDepth` / `canDrillBack` / `resolveDrillHit(addr)`；`drillInto(config, record)` 解析目标模板（失败设 `DRILL_RESOLVE_FAILED` 可读错误、停留当前报、栈不变）→ 目标模板默认值叠加映射参数 → 压栈切换并取数；`drillBack()` 弹栈恢复该层当时参数重新取数；宿主更换根模板时下钻栈作废；导出 / 打印始终作用于当前可见层。组件壳 `report-viewer.vue` 经 SheetGrid hit-test 把事件坐标解析为物理格：单击命中 `openMode: 'switch'` 下钻格触发 `drillInto`（pointerdown→click 位移超 4px 视为拖选抑制），悬停命中加 `drill-hover` 修饰给网格 `cursor: pointer`（`!important` 覆盖 VTable 内联 cursor），`canDrillBack` 时显示「返回上一层」drill-bar。组件级测试 `__test__/report-drill.test.ts`（stub connector + 三级下钻模板全流程）。
