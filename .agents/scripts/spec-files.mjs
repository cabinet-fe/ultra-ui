  ...
  for (const { lineNo, text } of body) {
    ...
    if (!match) {
      throw specError(
        specPath,
        lineNo,
        '每行必须是「- 新增|删除|修改：`路径`」，全角冒号，路径用反引号包裹'
      )
    }
...
function loadArchivedSpecs(specsDir) {
  ...
  for (const abs of collectArchivedSpecs(dir)) {
    ...
    out.push({ spec: rel, module: parts[0], files: parsed.files, title: readSpecTitle(abs) })
  }
  return out
}
...
function querySpecs(specsDir, files, { json = false } = {}) {
  ...
  for (const entry of specs) {
    ...
    if (hit.length > 0) {
      matches.push({ spec: entry.spec, module: entry.module, title: entry.title, hit })
    }
  }
  if (json) {
