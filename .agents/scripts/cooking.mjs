...
function parseStage(file) {
  ...
  for (const { index, text } of preLines) {
    ...
    if (!trimmed.startsWith('-'))
      fail(`${file}:${index + 1}: 「前置任务」每行须是「- 无」或「- P<n>」`)
...
function reviewFileIssues(unit) {
  ...
  for (const stage of unit.derived.ordered) {
    ...
    else if (conclusion !== review)
      issues.push(`${stage.id} 结论「${conclusion}」与评审「${review}」不一致`)
  }
  return issues
}
...
function nextStep(unit) {
  ...
  if (!unit.hasSpec)
    return unit.goal === '无' ? 'to-spec（无 goal，需求含糊则先 explore）' : 'to-spec'
...
function nextImpl(stage, stages, value) {
  ...
  if (value === '进行中') {
    if (impl === '完成' && review === '通过')
      fail(`${stage.id} 已评审通过，不可返工；需求变了请走 explore`)
    if (impl === '完成' && review === '未开始')
      fail(`${stage.id} 已实现完成、等待评审；返工须先评审为不通过`)
...
function setStatus(feature, id, field, value) {
  ...
  for (const [key, next] of Object.entries(changes)) {
    ...
    stage.lines[index] = stage.lines[index].replace(
      /^(\s*-\s*(?:实现|评审)\s*[：:]\s*)\S+/,
      `$1${next}`
    )
  }
