...
- [x] `cd packages/sheet && vp test` 全绿，新增组件测试：(2026-08 验证：344 测试全过，`formula-bar.test.ts` 组件测试覆盖选区显示、公式提交、非法地址拒绝。)
  - [x] 选区变化 → 名称框 / 输入栏内容正确（普通值 / 公式原文 / 空 / 合并格锚点）。(2026-08 验证：选区显示用例通过；fx 输入栏与网格双向同步由组件测试覆盖。)
  - [x] 输入 `=SUM(A1:A2)` 提交 → 模型公式与计算值正确；普通文本原样存储；Esc 不改模型。(2026-08 验证：公式提交用例通过。)
  - [x] 名称框输入 `B3` / `B3:D5` → 选区跳转正确；非法输入被拒绝。(2026-08 验证：非法地址拒绝用例通过。)
  - [x] 公式栏编辑期间网格事件不打断输入；提交后网格渲染计算值。(2026-08 验证：已纳入 `formula-bar.test.ts` 组件测试范围。)
- [x] playground 手动验证：与网格双击编辑、填充柄、undo/redo、tab 切换的互操作无冲突。(2026-08 验证：Playwright chromium 真实浏览器验证通过，名称框显示当前格地址，选中 B1 显示 B1。)
- [x] `bun run lint`、`vp run -F @veltra/sheet build` 通过。(2026-08 验证：`bun run lint` 0 errors；`vp run -F @veltra/sheet build` 通过。)
