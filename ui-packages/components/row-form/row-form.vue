  ...
  <u-table :data="rows" :columns="finalColumns">
    ...
    >
      <div
        ...
        @click="e => handleClick(e, row.index)"
      >
        <node-render
          :content="
      ...
      <template v-else-if="item.key === 'operation'">
        <button-common-props tag="span">
          <u-button :class="cls.m('interval')" :icon="Delete" type="primary" />

          <u-button :icon="DocumentAdd" type="primary" />
        </button-common-props>
      </template>
...
const handleBlurEvent = (e: Event) => {
  if (rows.value.length - 1 === obj.clickIndex.value) {
    let row = {}
    rows.value = wrapDataRows([...rows.value, row]) as T[]
  }
