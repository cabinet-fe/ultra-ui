...
> [!NOTE]
> 按钮整体采用现代的**全胶囊圆角（Pill shape, border-radius: 9999px）**外观设计，其左右内边距微调为 `1em`。Plain 朴素按钮已移除外层 `box-shadow`，呈现更为纯净轻量的视觉体验。此外，支持通过主题配置中的 `button['default-bg']` 定制默认按钮的背景色。
