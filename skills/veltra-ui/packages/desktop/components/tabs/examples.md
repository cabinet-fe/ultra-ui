...
function onClose(item: TabItem, index: number) {
  ...
  if (active.value === item.key) active.value = barItems.value[0]?.key ?? ''
}
</script>

<template>
  <u-tabs-horizontal :items="barItems" v-model="active" rounded closable block @close="onClose" />
  <u-tabs-vertical :items="barItems" v-model="active" rounded closable @close="onClose" />
</template>
```
