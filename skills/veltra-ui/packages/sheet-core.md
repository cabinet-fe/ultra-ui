...
- **保留**：值（数字 / 字符串 / 布尔 / 错误；日期 ↔ 1900 系统序列 `t='d'`）、**公式表达式**（`f` 不带 `=`，缓存值由本地引擎重算，不支持的函数 → `#ERROR!`）、合并、样式（fill + 四边 border + font + alignment + `numFmt` 四类 `date` / `thousands` / `cnUpper` / `fixed`，经 `StylePool.intern` 内容去重；border 线型 12 种收敛到模型 5 种；theme 色经工作簿调色板解析）、冻结、行高（points ↔ px ×4/3 取整）、**浮动图片**（字节 / 类型 / 锚点 / 宽高；WPS `cellImages` 单元格内嵌图跳过；CSV 忽略图片）。
- **不导入 / 忽略**：列宽（模型未持久化列宽）；四类之外的数字格式 `numFmt`。CSV 导原始值，不应用显示格式。
