...
function resolveStyleSideEffects(kebab) {
  const ext = 'ts'
  let kebabName = kebab
...
function ultraUIResolver() {
  return {
    ...
    resolve: (name) => {
      ...
      return { name, from: '@ultra-ui/pc', sideEffects }
    }
  }
}
export default defineConfig(() => {
  return {
    ...
    css: {
      preprocessorOptions: { scss: { loadPaths: [resolve(__dirname, '../packages/styles/src')] } }
    },
    plugins: [
      vue(),
      vueJsx(),
      Components({ resolvers: [ultraUIResolver()], dts: true }),
      UnoCSS(),
