...
- [ ] 实现顶部 Filter Bar 组件，根据绑定的数据集参数定义自动生成控件并联动报表重新填充渲染
- [ ] 在 `playground/src/sheet-report/` 中生成并集成 3 套商业预置 Tab 模板，可一键无缝切换预览
- [ ] 添加参数更改与报表计算时的加载指示器（Loading 遮罩）
- [ ] 集成保真 XLSX 文件导出功能，确认导出的 Excel 文件保留合并单元格、列宽行高及条件格式渲染颜色
